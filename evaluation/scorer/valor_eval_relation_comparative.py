...
class CHAIR(object):
    ...
    def __init__(self, args):
        ...
        for image_info in self.matched_objects_file:
            ...
            self.imageid_to_gt_objects[image_id] = image_info["gt_words"]
            self.imageid_to_ge_objects[image_id] = image_info["generated_words"]
            self.imageid_to_matched_objects[image_id] = image_info["matched_objects_filtered"]
            # self.imageid_to_mapping[image_id] = image_info["mapping"]
            self.imageid_to_broader_concept[image_id] = image_info["broader_concept"]
            self.broader_concept_filtered[image_id] = image_info["num_broader_concept_filtered"]
    ...
    def compute_chair(self):
        ...
        for image_id in imageids:
            ...
            gt_objects = self.imageid_to_gt_objects[image_id].values()
            if self.imageid_to_ge_objects[image_id] == "":
                ge_objects = []
            else:
                ge_objects = self.imageid_to_ge_objects[image_id].values()
            matched_objects = list(self.imageid_to_matched_objects[image_id])
            ...
            faithful_word_len = len(matched_objects) - self.broader_concept_filtered[image_id]   #+ len(set(broader_concepts.keys()))
            faithful_word_count += faithful_word_len
            faithful = faithful_word_len == len(ge_objects)

            # count covered words
            gt_word_count += len(set(gt_objects))
            covered_word_count += len(set(matched_objects)) - self.broader_concept_filtered[image_id]         
...
if __name__ == "__main__":
    ...
    parser.add_argument("-i","--matched_objects_file_path", type=str,required=True)
    parser.add_argument("-o","--evaluation_results_path", type=str, required=True)
